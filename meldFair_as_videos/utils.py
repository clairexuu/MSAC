...
def batch_to_all_labels(labels, lengths):
    """Convert batch labels to flattened format for loss calculation"""
    flat_labels = []
    batch_size = labels.size(1) if len(labels.shape) > 1 else labels.size(0)
    
    for j in range(batch_size):
        if len(labels.shape) > 1:
            flat_labels.append(labels[:lengths[j], j])
        else:
            flat_labels.append(labels[:lengths[j]])
    
    return torch.cat(flat_labels, dim=0)
