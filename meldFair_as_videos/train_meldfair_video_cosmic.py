...
def train_or_eval_model(model, loss_function, dataloader, epoch, optimizer=None, train=False, args=None):
    ...
    for i, batch in enumerate(dataloader):
        ...
        if torch.cuda.is_available() and not args.no_cuda:
            for key in ['audio', 'video_sequences', 'emotion_labels', 'sentiment_labels', 
        ...
        # Forward pass
        try:
            emotion_logits, sentiment_logits, shift_logits, fused_features = model(batch)
...
def main():
    parser = argparse.ArgumentParser(description='MELD-FAIR Video + COSMIC + GraphSmile Training')
    ...
    # Video parameters
    parser.add_argument('--max_frames', type=int, default=None, help='Maximum frames per video (None = all frames)')
    
    # Training parameters
    parser.add_argument('--batch_size', type=int, default=4, help='Batch size (smaller for video data)')
    ...
    model = MELDFAIRVideoCommonsenseGraphSmile(args, n_classes_emo=7, n_classes_sent=3)
    ...
    video_params = list(model.video_frontend.parameters()) + list(model.video_temporal.parameters())
    other_params = [p for n, p in model.named_parameters() 
                   if not any(name in n for name in ['video_frontend', 'video_temporal'])]
    
    optimizer = optim.AdamW([
        {'params': video_params, 'lr': args.lr * 0.1},  # Lower LR for video components
    ...
    print(f'\nVideo training completed. Best F1: {best_fscore:.2f}% at epoch {best_epoch+1}')
    
    # Load best model and test
    print('\nTesting best video model...')
